...
public class TestWorkflows {
    ...
    static final String SYSTEM_PROMPT = "你是 Apboa 智能体平台的 AI 助手。你的核心职责是帮助用户通过自然语言管理平台上的智能体和技能。\n" +
            "\n" +
            "## 你拥有的能力\n" +
            "\n" +
            "### 1. Apboa 平台 CLI 工具\n" +
            "通过 `opencli apboa` 命令管理平台资源（智能体、技能、工具、知识库等）。\n" +
            "当需要操作平台时，使用 Bash 工具执行 CLI 命令。所有命令默认输出 JSON 格式用 `-f json`。\n" +
            "\n" +
            ...
            "当没有合适的现有技能时，使用 skill-creator 从零创建新技能。\n" +
            "\n" +
            "## 核心工作流程\n" +
            "\n" +
            "### 工作流 A：创建智能体\n" +
            ...
            "2. **社区搜索** — 执行 `npx skills find <keyword>`\n" +
            ...
            "2. **创建技能** — 使用 skill-creator 的指引创建 SKILL.md 和相关资源\n" +
            "3. **测试验证** — 确保技能文件结构正确\n" +
            "4. **打包上传** — 将技能目录打包为 .zip 并上传到平台\n" +
            "\n" +
            "## 重要规则\n" +
            "1. **每一步都要向用户汇报进度** — 告诉用户当前在做什么\n" +
            "2. **遇到问题要询问用户** — 不要自己猜测不确定的信息\n" +
            "3. **创建智能体前必须确认** — 展示即将创建的配置，让用户确认\n" +
            "4. **使用中文回答**\n" +
            "5. **CLI 命令输出用 `-f json`** — 方便解析和展示\n" +
            "6. **打包 .zip 用 PowerShell** — 必须用完整 Windows 路径，示例：\n" +
            "   `powershell.exe -Command \"Compress-Archive -Path 'C:\\Users\\14420\\.agents\\skills\\code-review\\*' -DestinationPath 'C:\\Users\\14420\\code-review.zip' -Force\"`\n" +
            "   注意：不要用 $HOME 或 ~，必须用 `C:\\Users\\14420` 这样的完整路径\n" +
            "7. **创建智能体需要 skillIds 和 modelConfigId** — 必须先查询获取\n" +
            "8. **npx skills 安装路径** — `~/.agents/skills/<name>/`\n" +
            "9. **并行执行** — 能并行的操作尽量并行\n" +
            "10. **不要过度验证** — 上传成功后直接进行下一步，不要解压验证";
    ...
    public static void main(String[] args) {
        ...
        // 选择测试哪个工作流
        String workflow = args.length > 0 ? args[0] : "C";

        switch (workflow) {
            case "A":
                System.out.println("╔══════════════════════════════════════════════════╗");
                System.out.println("║  测试工作流 A: 创建智能体                          ║");
                System.out.println("╚══════════════════════════════════════════════════╝");
                System.out.println();
                System.out.println("用户: 帮我创建一个专门做代码审查的智能体，要求能检查代码质量、安全漏洞和性能问题");
                System.out.println();
                runWorkflow(chatModel, skillsTool,
                        "帮我创建一个专门做代码审查的智能体，要求能检查代码质量、安全漏洞和性能问题");
                break;

            case "B":
                System.out.println("╔══════════════════════════════════════════════════╗");
                System.out.println("║  测试工作流 B: 搜索并上传技能                      ║");
                System.out.println("╚══════════════════════════════════════════════════╝");
                System.out.println();
                System.out.println("用户: 帮我搜索一下有没有关于 Docker 部署的 skill，找到后上传到平台");
                System.out.println();
                runWorkflow(chatModel, skillsTool,
                        "帮我搜索一下有没有关于 Docker 部署的 skill，找到后上传到平台");
                break;

            case "C":
                System.out.println("╔══════════════════════════════════════════════════╗");
                System.out.println("║  测试工作流 C: 创建技能                            ║");
                System.out.println("╚══════════════════════════════════════════════════╝");
                System.out.println();
                System.out.println("用户: 帮我创建一个 SQL 查询生成器的 skill，用户输入自然语言描述，skill 帮他生成对应的 SQL 语句");
                System.out.println();
                runWorkflow(chatModel, skillsTool,
                        "帮我创建一个 SQL 查询生成器的 skill，用户输入自然语言描述，skill 帮他生成对应的 SQL 语句。创建完成后上传到 Apboa 平台。");
                break;

            default:
                System.out.println("未知工作流: " + workflow);
        }

        System.out.println();
        System.out.println("========================================");
        System.out.println("工作流 " + workflow + " 测试完成");
        System.out.println("========================================");
    }

    static void runWorkflow(ChatModel chatModel, Tool skillsTool, String userMessage) {
        ...
        for (int round = 0; round < maxRounds; round++) {
            ...
            for (ToolMessage tm : toolMessages) {
                ...
                if (tc != null && tc.length() > 400) {
                    System.out.println("  返回: " + tc.substring(0, 400) + "...");
                } else {
